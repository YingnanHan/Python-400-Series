...
# 新增功能：
#   1.实现我方坦克与敌方坦克不可以互相穿过
#   2.敌我双方坦克一旦碰撞二者不能互相继续移动
...
class Tank(BaseItem):
    ...
    # 移动
    def move(self):
        #修改当前坦克的坐标 值得注意的是如果发生碰撞这里的值就不会被修改
        #也就是说一旦发生碰撞，坦克的坐标要修改为碰撞发生前时的位置
        self.oldLeft = self.rect.left
        self.oldTop = self.rect.top
...
# 墙壁类
class Wall():
    #首先，墙壁实际上也是一张图片，需要对其进行位置的设置
    #left top 参数就是用于固定墙壁图片位置的
    def __init__(self,left,top):
        #加载墙壁图片
